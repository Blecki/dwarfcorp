    ...
    public class InstanceList
    {
        ...
        public void Render(GraphicsDevice graphics, Effect effect)
        {
            ...
            if (InstanceBuffer != null && !InstanceBuffer.IsDisposed)
            {
                ...
                foreach(InstancedVertex vertex in Vertices.Values)
                {
                    effect.Parameters["xWorld"].SetValue(vertex.Transform);
                    graphics.DrawPrimitives(PrimitiveType.TriangleList, 0, Model.VertexCount);
                }
                effect.CurrentTechnique = effect.Techniques["Textured"];
