...
sampler IllumSampler = sampler_state { texture = <xIllumination> ;  magfilter = POINT; minfilter = POINT; mipfilter=POINT; AddressU = clamp; AddressV = clamp; AddressW = clamp; };

sampler TextureSampler = sampler_state { texture = <xTexture> ; magfilter = POINT; minfilter = POINT; mipfilter=POINT; AddressU = clamp; AddressV = clamp; AddressW = clamp;};Texture xTexture0;
Texture xTexture1;

sampler TextureSampler1 = sampler_state { texture = <xTexture1> ; magfilter = POINT; minfilter = POINT; mipfilter=POINT; AddressU = clamp; AddressV = clamp; AddressW = clamp;};
Texture xTexture2;

sampler TextureSampler2 = sampler_state { texture = <xTexture2> ; magfilter = POINT; minfilter = POINT; mipfilter=POINT; AddressU = clamp; AddressV = clamp; AddressW = clamp;};
Texture xTexture3;

sampler TextureSampler3 = sampler_state { texture = <xTexture3> ; magfilter = POINT; minfilter = POINT; mipfilter=POINT; AddressU = clamp; AddressV = clamp; AddressW = clamp;};
...
TVertexToPixel TexturedVS( float4 inPos : POSITION,  float2 inTexCoords: TEXCOORD0, float4 inColor : COLOR0, float4 inTexSource : TEXCOORD1, float4x4 world : BLENDWEIGHT, float4 tint : COLOR1)
{
	...
	Output.Fog = saturate((Output.Position.z - 50) / (80 - 50)); //saturate((Output.Position.z - xFogStart) / (xFogEnd - xFogStart));
...
TPixelToFrame TexturedPS_Alphatest(TVertexToPixel PSIn)
{
	...
	Output.Color.rgb = lerp(Output.Color.rgb, xFogColor, PSIn.Fog);
...
{    
	 ...
	 Output.Color = inColor;
	 Output.Fog = saturate((Output.Position.z - 50) / (80 - 50));
...
WPixelToFrame WaterPS(WVertexToPixel PSIn)
{
    ...
    float4 sloshColor = tex2D(WrappedTextureSampler1, PSIn.UnMovedTextureSamplingPos + perturbation);
    ...
    Output.Color.rgba = float4(lerp(Output.Color.rgb, xFogColor, PSIn.Fog), Output.Color.a);
