...
namespace DwarfCorp
{
    ...
    {
        Dictionary<string, Timer> splashNoiseLimiter = new Dictionary<string, Timer>();
        ChunkManager Chunks { get; set; }
        ...
        public static Vector3[] m_spreadNeighbors = { new Vector3(1, 0,  0), new Vector3(-1, 0,  0), new Vector3(0, 0, 1), new Vector3(0, 0, -1) };
        ...
        public WaterManager(ChunkManager chunks)
        {
            ...
            splashNoiseLimiter["bubble2"] = new Timer(0.1f, false);
        ...
        public void CreateSplash(Vector3 pos, LiquidType liquid)
        {
            if (liquid == LiquidType.Water)
            {
                ...
                splash.name = "bubble2";
                splash.numSplashes = 2;
                splash.position = pos;
                splash.sound = "river";
                Splashes.Enqueue(splash);
            }
            else if (liquid == LiquidType.Lava)
            {
                SplashType splash = new SplashType();
                splash.name = "flame";
                splash.numSplashes = 5;
                splash.position = pos;
                splash.sound = "fire";
                Splashes.Enqueue(splash);
            }
        ...
        public void UpdateWater()
        {
            ...
            List<VoxelChunk> chunksToUpdate = new List<VoxelChunk>();
            foreach (KeyValuePair<Point3, VoxelChunk> chunks in Chunks.ChunkMap)
            {
                chunksToUpdate.Add(chunks.Value);
            }
        ...
        {
            List<VoxelRef> voxelsBelow = new List<VoxelRef>();
            ...
            {
                Point3 point = updateList[indices[i]];
                int x = point.X;
                int y = point.Y;
                int z = point.Z;
