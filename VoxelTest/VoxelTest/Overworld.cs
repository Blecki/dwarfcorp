...
namespace DwarfCorp
{
    public class Overworld
    {
        ...
        static Vector2[] deltas2d = { new Vector2(-1, 0), new Vector2(1, 0), new Vector2(0, -1), new Vector2(0, 1) };
        ...
        static Perlin xDistort = new Perlin(PlayState.random.Next());
        static Perlin yDistort = new Perlin(PlayState.random.Next());
        ...
        public static float LinearInterpolate(Vector2 position, MapData[,] map, ScalarFieldType fieldType)
        {
            float x = position.X;
            float y = position.Y;
            float x1 = (int)LinearMathHelpers.Clamp((float)Math.Ceiling(x), 0, map.GetLength(0) - 2);
            float y1 = (int)LinearMathHelpers.Clamp((float)Math.Ceiling(y), 0, map.GetLength(1) - 2);
            float x2 = (int)LinearMathHelpers.Clamp((float)Math.Floor(x), 0, map.GetLength(0) - 2);
            float y2 = (int)LinearMathHelpers.Clamp((float)Math.Floor(y), 0, map.GetLength(1) - 2);
            ...
            return LinearMathHelpers.LinearCombination(x, y, x1, y1, x2, y2, q11, q22, q21, q22);
        }
        ...
        public static float Interpolate(float wx, float wy,  float globalScale, float[,] map)
        {
            ...
            return LinearMathHelpers.LinearInterpolate(new Vector2(x, y), map);
        }
