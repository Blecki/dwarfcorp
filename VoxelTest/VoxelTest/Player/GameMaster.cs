...
namespace DwarfCorp
{
    ...
    public class GameMaster
    {
            ...
            switch(this.CurrentTool)
            {
                ...
                case ToolMode.CreateStockpiles:
                    foreach(Stockpile s in Stockpiles)
                    {
                        ...
                        {
                            SimpleDrawing.DrawBox(storage.Voxel.GetBoundingBox(), storage.Voxel == null ? Color.White : Color.Yellow, 0.05f, true);
                        }
                    }
                    break;
                case ToolMode.Chop:
        ...
        public void Update(DwarfGame game, GameTime time)
        {
            ...
            {
                VoxelRef vref = d.vox;
                Voxel v = vref.GetVoxel(Chunks, false);
        ...
        public Designation GetGuardDesignation(Voxel vox)
        {
            return (from d in GuardDesignations
                let vref = d.vox
                let v = vref.GetVoxel(Chunks, false)
                where vox == v
        ...
        {
            return DigDesignations.Select(d => d.vox).Select(vref => vref.GetVoxel(Chunks, false)).Any(v => vox == v);
        }
        ...
        {
            return GuardDesignations.Select(d => d.vox).Select(vref => vref.GetVoxel(Chunks, false)).Any(v => vox == v);
        }
        ...
        public void GatherMode(MouseState mouseState, KeyboardState keyState, DwarfGame game, GameTime time)
        {
            ...
            foreach(LocatableComponent resource in resourcesPickedByMouse.Where(resource => resource.IsActive && resource.IsVisible && resource.Parent == Components.RootComponent && !IsInStockpile(resource)))
            {
        ...
        public void OnSelected(List<VoxelRef> refs, InputManager.MouseButton button)
        {
            ...
            switch(CurrentTool)
            {
                case ToolMode.Dig:
                    ...
                    else
                    {
                        foreach(VoxelRef r in refs)
                        {
                            ...
                            foreach(CreatureAIComponent minion in Minions.Where(minion => minion != null && minion.Goap != null))
                            {
                ...
                case ToolMode.Guard:
                    foreach(Voxel v in from r in refs
                        where r != null
                        select r.GetVoxel(Chunks, false))
                    {
                        if(button == InputManager.MouseButton.Left)
                        {
                ...
                case ToolMode.CreateStockpiles:
                    ...
                    foreach(Voxel v in refs.Where(r => r != null).Select(r => r.GetVoxel(Chunks, false)).Where(v => v != null))
                    {
                        if(button == InputManager.MouseButton.Left)
                        {
