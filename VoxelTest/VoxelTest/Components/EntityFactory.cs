...
namespace DwarfCorp
{
    ...
    internal class EntityFactory
    {
        ...
        public static GameComponent GenerateTree(float treeSize, Vector3 position, float offset, ComponentManager componentManager, ContentManager content, GraphicsDevice graphics, string asset)
        {
            ...
            matrix.Translation = position;
            Texture2D spriteSheet = content.Load<Texture2D>(asset);
            LocatableComponent tree = new LocatableComponent(componentManager, asset, componentManager.RootComponent, matrix, new Vector3(treeSize * 2, treeSize * 3, treeSize * 2), Vector3.Zero);
            ModelInstanceComponent modelInstance = new ModelInstanceComponent(componentManager, "Model", tree, Matrix.CreateRotationY((float)(PlayState.Random.NextDouble() * Math.PI)) * Matrix.CreateScale(treeSize * 4, treeSize * 4, treeSize * 4) * Matrix.CreateTranslation(new Vector3(0.5f, treeSize * offset, 0.5f)), asset, false);
        ...
        public static GameComponent GenerateBerryBush(float bushSize, Vector3 position, float offset, ComponentManager componentManager, ContentManager content, GraphicsDevice graphics)
        {
            ...
            ModelInstanceComponent modelInstance = new ModelInstanceComponent(componentManager, "Model", tree, Matrix.CreateScale(bushSize, bushSize, bushSize) * Matrix.CreateTranslation(new Vector3(0.5f, bushSize * offset - 0.1f, 0.5f)), "berrybush", false);
            ...
            GameMaster master, PlanService planService, string allies)
        {
            CreatureStats stats = new CreatureStats
            {
                ...
                EnergyLoss = 0.0005f,
                EnergyRecharge = 0.1f,
                EnergyRechargeBed = 0.005f,
                HungerIncrease = 0.005f,
                HungerThreshold = 0.8f,
                SleepyThreshold = 0.15f,
                PlanRateLimit = 0.1f,
                MaxExpansions = 20000
            };
