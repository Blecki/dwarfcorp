...
namespace DwarfCorp
{
    class EntityFactory
    {
                                                  ...
                                                  GameMaster master, PlanService planService, string allies)
        {
            ...
            Matrix matrix = Matrix.Identity;
            matrix.Translation = position;
            testDwarf = new PhysicsComponent(componentManager, "dwarf", componentManager.RootComponent, matrix,  new Vector3(0.5f, 0.5f, 0.5f), new Vector3(0.0f, -0.25f, 0.0f),  1.0f, 1.0f, 0.999f, 0.999f, new Vector3(0, -10, 0));
            ...
            Point offset = dwarfOffsets[PlayState.random.Next(0, dwarfOffsets.Count)];
            ...
            testSprite = new OrientedAnimation(componentManager, "testsprite", testDwarf, spriteMatrix);
            Texture2D swimSheet = content.Load<Texture2D>("dorf_splash");
            testSprite.AddAnimation("Walk", dwarfWalkRight, dwarfWalkLeft, dwarfWalkForward, dwarfWalkBack);
            testSprite.AddAnimation("Idle", dwarfIdleRight, dwarfIdleLeft, dwarfIdleForward, dwarfIdleBack);
            testSprite.AddAnimation("Swim", new Animation(dwarfWalkRight, swimSheet, graphics), new Animation(dwarfWalkLeft, swimSheet, graphics), new Animation(dwarfWalkForward, swimSheet, graphics), new Animation(dwarfWalkBack, swimSheet, graphics));



#region pickaxe
            List<Point> pickaxeRightFrames = new List<Point>();
            pickaxeRightFrames.Add(new Point(0,0));
            pickaxeRightFrames.Add(new Point(2,0));
            pickaxeRightFrames.Add(new Point(1,0));
            pickaxeRightFrames.Add(new Point(0,0));

            Animation pickaxeRight = new Animation(graphics, content.Load<Texture2D>("pickaxe"), "pickaxe_right", 28, 28, pickaxeRightFrames, true, Color.White, 15.0f, 1.0f, 1.0f, true);

            List<Point> pickaxeLeftFrames = new List<Point>();
            pickaxeLeftFrames.Add(new Point(0,0));
            pickaxeLeftFrames.Add(new Point(2,0));
            pickaxeLeftFrames.Add(new Point(1,0));
            pickaxeLeftFrames.Add(new Point(0,0));

            Animation pickaxeLeft = new Animation(graphics, content.Load<Texture2D>("pickaxe"), "pickaxe_left", 28, 28, pickaxeLeftFrames, true, Color.White, 15.0f, 1.0f, 1.0f, false);


             List<Point> pickaxeForwardFrames = new List<Point>();
            pickaxeForwardFrames.Add(new Point(0,1));
            pickaxeForwardFrames.Add(new Point(2,1));
            pickaxeForwardFrames.Add(new Point(1,1));
            pickaxeForwardFrames.Add(new Point(0,1));

            Animation pickaxeForward = new Animation(graphics, content.Load<Texture2D>("pickaxe"), "pickaxe_forward", 28, 28, pickaxeForwardFrames, true, Color.White, 15.0f, 1.0f, 1.0f, true);

            
             List<Point> pickaxeBackwardFrames = new List<Point>();
            pickaxeBackwardFrames.Add(new Point(0,1));
            pickaxeBackwardFrames.Add(new Point(2,1));
            pickaxeBackwardFrames.Add(new Point(1,1));
            pickaxeBackwardFrames.Add(new Point(0,1));

            Animation pickaxeBackward= new Animation(graphics, content.Load<Texture2D>("pickaxe"), "pickaxe_backward", 28, 28, pickaxeBackwardFrames, true, Color.White, 15.0f, 1.0f, 1.0f, false);
#endregion

            List<Point> pickaxeRightFramesI = new List<Point>();
            pickaxeRightFramesI.Add(new Point(0,0));


            Animation pickaxeRightI = new Animation(graphics, content.Load<Texture2D>("pickaxe"), "pickaxe_right_idle", 28, 28, pickaxeRightFramesI, true, Color.White, 15.0f, 1.0f, 1.0f, true);

            List<Point> pickaxeLeftFramesI = new List<Point>();
            pickaxeLeftFramesI.Add(new Point(0,0));

            Animation pickaxeLeftI = new Animation(graphics, content.Load<Texture2D>("pickaxe"), "pickaxe_left_idle", 28, 28, pickaxeLeftFramesI, true, Color.White, 15.0f, 1.0f, 1.0f, false);


             List<Point> pickaxeForwardFramesI = new List<Point>();
            pickaxeForwardFramesI.Add(new Point(0,1));


            Animation pickaxeForwardI = new Animation(graphics, content.Load<Texture2D>("pickaxe"), "pickaxe_forward_idle", 28, 28, pickaxeForwardFramesI, true, Color.White, 15.0f, 1.0f, 1.0f, true);

            
             List<Point> pickaxeBackwardFramesI = new List<Point>();
            pickaxeBackwardFramesI.Add(new Point(0,1));


            Animation pickaxeBackwardI= new Animation(graphics, content.Load<Texture2D>("pickaxe"), "pickaxe_backward_idle", 28, 28, pickaxeBackwardFramesI, true, Color.White, 15.0f, 1.0f, 1.0f, false);

            Matrix weaponMatrix = spriteMatrix;
            weaponMatrix.Translation += new Vector3(0.1f, -0.05f, 0.0f);

            OrientedAnimation weaponAnimation = new OrientedAnimation(componentManager, "Pickaxe", testDwarf, weaponMatrix);
            weaponAnimation.AddAnimation("Attack", pickaxeRight, pickaxeLeft, pickaxeForward, pickaxeBackward);
            weaponAnimation.AddAnimation("Idle", pickaxeRightI, pickaxeLeftI, pickaxeForwardI, pickaxeBackwardI);
            ...
            Grabber hands = new Grabber(componentManager, "hands", testDwarf, Matrix.Identity, new Vector3(0.1f, 0.1f, 0.1f), Vector3.Zero);

            EnemySensor enemySensor = new EnemySensor(componentManager, "EnemySensor", testDwarf, Matrix.Identity, new Vector3(20, 5, 20), Vector3.Zero);       

            CreatureAIComponent dwarfAI = new CreatureAIComponent(componentManager, testDwarf, "Dwarf AI", testDwarf, testSprite, master, stats, hands, enemySensor, planService, allies);
            


            Weapon pickaxe = new Weapon("Pickaxe", 1.0f, 2.0f, 1.0f, dwarfAI, "pick");

            HealthComponent health = new HealthComponent(componentManager, "Health", testDwarf, stats.MaxHealth, 0.0f, stats.MaxHealth);

            Matrix shadowTransform = Matrix.CreateRotationX((float)Math.PI * 0.5f);
            shadowTransform.Translation = new Vector3(0.0f, -0.5f, 0.0f);
            ShadowComponent shadow = new ShadowComponent(componentManager, "Shadow", testDwarf, shadowTransform, content.Load<Texture2D>("shadowcircle")); 
            ...
            testDwarf.Tags.Add("Dwarf");

            EmitterComponent deathEmitter = new EmitterComponent("blood_particle", componentManager, "Death Gibs", testDwarf, Matrix.Identity, Vector3.One, Vector3.Zero);
