...
{
    public class Pipe : CraftedBody
    {
        [EntityFactory("Pipe")]
        private static GameComponent __factory6(ComponentManager Manager, Vector3 Position, Blackboard Data)
        {
            return new Pipe(Manager, Position, Data.GetData<Resource>("Resource", null));
        }
        ...
        public Pipe(ComponentManager manager, Vector3 position, Resource Resource) :
            base(manager, "Steam Pipe", Matrix.Identity, new Vector3(1.0f, 1.0f, 1.0f), Vector3.Zero, new DwarfCorp.CraftDetails(manager, Resource))
        {
            ...
            AddChild(new PipeNetworkObject(manager));
