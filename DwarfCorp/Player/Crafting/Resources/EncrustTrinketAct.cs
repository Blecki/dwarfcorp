...
namespace DwarfCorp
{
    internal class EncrustTrinketAct : CompoundCreatureAct
    {
        ...
        public IEnumerable<Status> DestroyResources(Func<Vector3> pos)
        {
            ...
            {
                var stashed = Agent.Blackboard.GetData<List<Resource>>("stashed-base-trinket").Concat(Agent.Blackboard.GetData<List<Resource>>("stashed-materials"));
        ...
        {
            ActualCreatedResource = Library.CreateMetaResource(ItemType.CraftActBehavior, Agent, new Resource(ItemType.ResourceCreated),
                Agent.Blackboard.GetData<List<Resource>>("stashed-base-trinket").Concat(Agent.Blackboard.GetData<List<Resource>>("stashed-materials")).ToList());
        ...
        public EncrustTrinketAct(CreatureAI creature, CraftItem CraftItem, Resource BaseTrinket, ResourceTypeAmount Gem, ResourceDes Des) :
            base(creature)
        {
            this.ItemType = CraftItem;
            this.BaseTrinket = BaseTrinket;
            this.Gem = Gem;
            this.Des = Des;
        ...
        public override void Initialize()
        {
            ...
            var getResources = new Sequence(
                new GetSpecificResourceAct(Agent, BaseTrinket) { BlackboardEntry = "stashed-base-trinket" },
                new Select(new Domain(() => Des.HasResources || Des.ResourcesReservedFor != null, true),
                                          new Domain(() => !Des.HasResources && (Des.ResourcesReservedFor == Agent || Des.ResourcesReservedFor == null),
                                                new Sequence(
                                                    new Wrap(ReserveResources),
                                                    new GetResourcesOfType(Agent, new List<ResourceTypeAmount> { Gem }) { BlackboardEntry = "stashed-materials" })
                                                | (new Wrap(UnReserve))
                                                & false),
