...
namespace DwarfCorp
{
    ...
    public class VoxelListPrimitive : GeometricPrimitive, IDisposable
    {
        ...
        {
            return v == VoxelVertex.BackTopLeft || v == VoxelVertex.FrontTopLeft || v == VoxelVertex.FrontTopRight ||
                   v == VoxelVertex.BackTopRight;
        }
                ...
                for (int y = 0; y < chunk.SizeY; y++)
                {
                    for (int z = 0; z < chunk.SizeZ; z++)
                    {
                        ...
                        foreach (VoxelVertex bestKey in top)
                        {
                            ...
                            chunk.GetNeighborsSuccessors(neighbors, (int) v.GridPosition.X, (int) v.GridPosition.Y,
                                (int) v.GridPosition.Z, diagNeighbors);
        ...
        {
            var faces = (BoxFace[]) Enum.GetValues(typeof (BoxFace));

            var ramps = new List<RampType>
            {
                RampType.None,
            ...
            var myTop = new Dictionary<VoxelVertex, float>
            {
                {
            ...
            var theirTop = new Dictionary<VoxelVertex, float>
            {
                {
                    VoxelVertex.BackTopLeft, 0.0f
                },
            ...
            foreach (RampType myRamp in ramps)
            {
                ...
                foreach (RampType neighborRamp in ramps)
                {
                    ...
                    foreach (BoxFace neighborFace in faces)
                    {
                        ...
                        FaceDrawMap[(int) neighborFace, (int) myRamp, (int) neighborRamp] = (their1 < my1 ||
                                                                                             their2 < my2);
                    }
                }
            }
        ...
        {
            var faceExists = new Dictionary<BoxFace, bool>();
            var faceVisible = new Dictionary<BoxFace, bool>();
            Voxel v = chunk.MakeVoxel(0, 0, 0);
            Voxel vAbove = chunk.MakeVoxel(0, 0, 0);
            Voxel voxelOnFace = chunk.MakeVoxel(0, 0, 0);
            var worldVoxel = new Voxel();
                    ...
                    for (int z = 0; z < chunk.SizeZ; z++)
                    {
                        ...
                        if (isTop && !v.IsEmpty && v.IsVisible && v.Type.CanRamp)
                        {
                            for (int i = 0; i < 6; i++)
                            {
                                ...
                                faceExists[face] = chunk.IsCellValid(x + (int) delta.X, y + (int) delta.Y,
                                    z + (int) delta.Z);
                                ...
                                {
                                    if (
                                        !chunk.Manager.ChunkData.GetNonNullVoxelAtWorldLocation(
                                            new Vector3(x + (int) delta.X + 0.5f, y + (int) delta.Y + 0.5f,
                                                z + (int) delta.Z + 0.5f) + chunk.Origin, ref worldVoxel) ||
        ...
        public void InitializeFromChunk(VoxelChunk chunk, GraphicsDevice graphics)
        {
            ...
            isRebuilding = true;
            rebuildMutex.ReleaseMutex();
            var ambientValues = new int[4];
            int maxIndex = 0;
            int maxVertex = 0;
            Voxel v = chunk.MakeVoxel(0, 0, 0);
            Voxel voxelOnFace = chunk.MakeVoxel(0, 0, 0);
            var manhattanNeighbors = new Voxel[4];
                ...
                for (int x = 0; x < chunk.SizeX; x++)
                {
                    for (int z = 0; z < chunk.SizeZ; z++)
                    {
                        ...
                        for (int i = 0; i < 6; i++)
                        {
                            var face = (BoxFace) i;
                            Vector3 delta = FaceDeltas[(int) face];
                            faceExists[(int) face] = chunk.IsCellValid(x + (int) delta.X, y + (int) delta.Y,
                                z + (int) delta.Z);
                            ...
                            if (faceExists[(int) face])
                            {
                                voxelOnFace.GridPosition = new Vector3(x + (int) delta.X, y + (int) delta.Y,
                                    z + (int) delta.Z);
                                drawFace[(int) face] = (voxelOnFace.IsExplored && voxelOnFace.IsEmpty) ||
                                                       !voxelOnFace.IsVisible ||
                                                       (voxelOnFace.Type.CanRamp &&
                                                        voxelOnFace.RampType != RampType.None && IsSideFace(face) &&
                            ...
                            else
                            {
                                bool success =
                                    chunk.Manager.ChunkData.GetNonNullVoxelAtWorldLocation(
                                        new Vector3(x + (int) delta.X, y + (int) delta.Y, z + (int) delta.Z) +
                                        chunk.Origin, ref worldVoxel);
                                drawFace[(int) face] = !success || (worldVoxel.IsExplored && worldVoxel.IsEmpty) ||
                                                       !worldVoxel.IsVisible ||
                        ...
                        for (int i = 0; i < 6; i++)
                        {
                            ...
                            int vertexIndex = 0;
                            int vertexCount = 0;
                            primitive.GetFace(face, out faceIndex, out faceCount, out vertexIndex, out vertexCount);
                            ...
                            for (int vertOffset = 0; vertOffset < vertexCount; vertOffset++)
                            {
                                ...
                                {
                                    var newVertices = new ExtendedVertex[Vertices.Length*2];
                            ...
                            for (int idx = faceIndex; idx < faceCount + faceIndex; idx++)
                            {
                                ...
                                {
                                    var indexes = new ushort[Indexes.Length*2];
                                    Indexes.CopyTo(indexes, 0);
                                    Indexes = indexes;
                                }

                                ushort vertexOffset = flippedQuad
                                    ? primitive.FlippedIndexes[idx]
                                    : primitive.Indexes[idx];
                                ushort vertexOffset0 = flippedQuad
                                    ? primitive.FlippedIndexes[faceIndex]
                                    : primitive.Indexes[faceIndex];
                                Indexes[maxIndex] =
        ...
        public bool ContainsNearVertex(Vector3 vertexPos1, List<VertexPositionColorTexture> accumulated)
        {
            ...
            return accumulated.Select(vert => (vertexPos1 - (vert.Position)).LengthSquared())
                .Any(dist => dist < epsilon);
        }
    }
}