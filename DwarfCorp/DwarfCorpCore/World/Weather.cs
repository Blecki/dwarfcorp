...
namespace DwarfCorp
{
    public class Weather
    {
        ...
        public void Update()
        {
            DateTime currentDate = World.Time.CurrentDate;
        ...
        public class Storm
        {
                ...
                {
                    {
                        ...
                        {
                            RainEffect = World.ParticleManager.Effects["rain"],
                            HitEffect = World.ParticleManager.Effects["splat"],
                            RainSpeed = 30,
                            CreatesLiquid = true,
                    ...
                    },
                    {
                        ...
                        {
                            RainEffect = World.ParticleManager.Effects["snowflake"],
                            HitEffect = World.ParticleManager.Effects["snow_particle"],
                            RainSpeed = 10,
                            RainRandom = 10f,
                            CreatesVoxel = true,
            ...
            public void Start()
            {
                World.AnnouncementManager.Announce("A storm is coming!", "A storm is incoming.");
        ...
        public static List<Storm> CreateForecast(int days)
        {
            ...
            DateTime date = World.Time.CurrentDate;
            for (int i = 0; i < days; i++)
            {
                // Each day, a storm could originate from a randomly selected biome
                Vector3 randomSample = MathFunctions.RandVector3Box(World.ChunkManager.Bounds);
        ...
        public class Cloud : Fixture
        {
            ...
            public Cloud(float raininess, int maxRain, float height, Vector3 pos) :
                base(pos, new SpriteSheet(ContentPaths.Particles.stormclouds), new Point(0, 0), World.ComponentManager.RootComponent)
            {
            ...
            public override void Update(DwarfTime gameTime, ChunkManager chunks, Camera camera)
            {
                ...
                for (int i = 0; i < MaxRainDrops; i++)
                {
                    ...
                    if (!World.ChunkManager.ChunkData.GetVoxel(RainDrops[i].Pos, ref test)) continue;
